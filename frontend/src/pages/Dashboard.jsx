...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Load hot leads
    api.leads.list({ limit: 3 }).then(leads => {
      const hot = leads.filter(l => l.ai_score === 3);
      setHotLeadsList(hot.slice(0, 3));
    }).catch(() => {});

    // Load latest drafts
    api.drafts.list({ limit: 3 }).then(setLatestDrafts).catch(() => {});
    ...
    <div className="p-6 max-w-7xl mx-auto" data-testid="dashboard-page">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Your agents are monitoring intent signals 24/7</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border p-5" data-testid="total-leads-card">
          ...
          <div className="text-xs text-gray-400 mt-1">From intent signals</div>
        </div>

        <div className="bg-white rounded-xl border p-5" data-testid="hot-leads-card">
          <div className="flex items-center gap-2 mb-2">
        ...
        <div className="bg-white rounded-xl border p-5" data-testid="contacted-card">
          <div className="flex items-center gap-2 mb-2">
          ...
          <div className="text-xs text-gray-400 mt-1">AI-generated emails</div>
        </div>

        <div className="bg-white rounded-xl border p-5" data-testid="replies-card">
          <div className="flex items-center gap-2 mb-2">
          ...
          <div className="text-xs text-gray-400 mt-1">Approved & sent</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border p-5">
          ...
          {hotLeadsList.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">No hot leads yet - configure your signals to start capturing intent!</p>
          ) : (
        ...
        <div className="bg-white rounded-xl border p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-gray-900">Latest AI Drafts</h3>
              </div>
              <p className="text-xs text-gray-400">AI-generated outreach emails</p>
            </div>
            <button className="text-xs text-orange-500 font-medium flex items-center gap-1">
              View More <ChevronRight className="w-3 h-3" />
            </button>
          ...
          {latestDrafts.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">No drafts yet - leads will generate AI emails automatically!</p>
          ) : (
            <div className="space-y-3">
              {latestDrafts.map((draft, i) => (
                <div key={i} className="flex items-center gap-3 py-2 border-b last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-gray-900">{draft.lead_name || 'Unknown'}</div>

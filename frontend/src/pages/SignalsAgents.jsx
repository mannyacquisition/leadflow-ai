...
// ── 3-dot Actions Menu ───────────────────────────────────────────────────────
function ActionsMenu({ agent, onRefresh }) {
  ...
  const handlePause = async () => {
    ...
    await api.signals.updateStatus(agent.id, newStatus);
  ...
  const handleDuplicate = async () => {
    const { id, user_id, created_at, updated_at, ...rest } = agent;
    await api.signals.create({ ...rest, name: `${agent.name} (Copy)` });
  ...
  const handleDelete = async () => {
    await api.signals.delete(agent.id);
...
export default function SignalsAgents() {
  ...
  return (
    <div className="h-full flex flex-col bg-white" data-testid="signals-agents-page">
      {/* Header */}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span className="text-gray-700">{agents.length} | 10 max</span>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {activeAgent && (
          <div className="border-2 rounded-xl p-5 max-w-sm" style={{ borderColor: "#e5e7eb" }}>
            ...
            <p className="text-xs text-gray-400 mb-3">Monitoring intent signals</p>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                agents.map(agent => (
                  <tr key={agent.id} className="border-b last:border-0 hover:bg-gray-50" data-testid={`agent-row-${agent.id}`}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                            <Loader2 className="w-3 h-3 animate-spin" /> Monitoring signals...
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <ActionsMenu agent={agent} onRefresh={fetchAgents} />
                      </div>
                    </td>
                  </tr>
                ))
